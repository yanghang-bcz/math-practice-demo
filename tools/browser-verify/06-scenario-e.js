(() => {
  ...
  ck('提示语是「判题服务这次没能给出结论」', toasts.some(t => t.includes('没能给出结论')), JSON.stringify(toasts));
  ...
  ck('两种原因确实被区分开了',
    all.some(t => t.includes('没能给出结论')) && all.some(t => t.includes('连不上判题服务')),
    JSON.stringify(all));

  ck('题目仍未被作废', !q || q.status !== 'void', q && q.status);
  ck('答案仍在输入框里', input && input.value === 'sin(x)', input && JSON.stringify(input.value));
